...
# Initialisting the modules in pygame
pygame.init()
...
def display_obstacle(height):
    ...
    pygame.draw.rect(SCREEN, OBSTACLE_COLOR, (obstacle_x, height + 150,
                     OBSTACLE_WIDTH, 690 - height - 150))

# COLLISION DEXTECTION


def collision_detection(obstacle_x, obstacle_height, bird_y, bottom_obstacle_height):
    if obstacle_x >= -20 and obstacle_x <= 50 + 64:
...
def game_over():
    ...
    # game over
    display1 = game_over_font1.render(f'GAME OVER', True, (255, 255, 255))
    SCREEN.blit(display1, (53, 200))
    # shows your current score and your max score
    display2 = game_over_font2.render(
        f'Score: {score} - Max Score: {maximum}', True, (255, 255, 255))
    SCREEN.blit(display2, (70, 300))
...
while running:
    clockobject.tick(50)
    SCREEN.fill((0, 0, 0))  # Screen is filled with black color
    ...
    # COLLISION
    collision = collision_detection(obstacle_x, OBSTACLE_HEIGHT,
                                    bird_y, OBSTACLE_HEIGHT+150)
